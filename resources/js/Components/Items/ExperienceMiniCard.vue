<template>
    <Link :href="route('show.services', product.slug)"
        class="flex flex-col shadow-slate-400 shadow-lg inset-0 flex-shrink-0  h-full  rounded-tr-[3rem] rounded-bl-[3rem]  bg-white bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(120,119,198,0.3),rgba(255,255,255,0))] md:scale-90 hover:scale-100">
    <div class="flex flex-col">
        <!-- {{ currentImage }} -->
        <img :src="currentImage == '/laravel/public/' ? 'https://avcolombia.net/images/logo.webp' : currentImage"
            alt="Portada" class="h-48 w-full object-cover object-center rounded-tr-[3rem] rounded-bl-[3rem] " />
