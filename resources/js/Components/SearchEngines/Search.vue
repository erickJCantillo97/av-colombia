<template>
  <div class="flex ">
          ...
          <div class="p-4">
            ...
            <h3 class="text-sm font-extrabold uppercase text-gray-400">Filtro por Servicio</h3>
              <div class="flex gap-x-2 py-1">
                <div v-for="t in servicesType" @click="type = t" :class="type.value == t.value ? 'bg-gray-600 text-white':''" class="text-xs text-gray-700 rounded-lg border py-1 px-1.5 font-semibold cursor-pointer select-none">
                  {{ t.label }}
                </div>
              </div>
            </div>
                ...
                <li @click="generateWithAi"
                  ...
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p class="text-sm text-gray-500">Usar IA para buscar en nuestra base de datos<span v-if="type.value"> de {{ type.label }}</span>.</p>
                    </div>
                  </div>
                </li>
              </ul>
    ...
    <Transition name="modal">
      <div v-if="isModalVisible" class="absolute inset-0 z-[99999999] flex items-center justify-center bg-black bg-opacity-50">
...
const generateWithAi = async () => {
  ...
  try {
    const response = await axios.post('/get-service', { prompt: searchQuery.value, type: type.value.value });
