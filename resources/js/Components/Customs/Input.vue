...
if (props.type == "country") {
  axios
    .get("https://restcountries.com/v3.1/all?fields=flags,translations,name")
    .then((res) => {
...
const format = (date) => {
  ...
  return `${padZero(day)}/${padZero(month)}/${year} ${padZero(date.getHours())}:${padZero(
    date.getMinutes()
  )}`;
};
...
<template>
  <div class="flex flex-col">
    ...
    <span v-else>
      <label v-if="label && !floatLabel" :for="id" class="mb-0.5 font-bold">{{
        label
      }}</label>
      <span :class="!(label && !floatLabel) ? 'p-float-label' : undefined">
        <InputNumber
          ...
          :required
          :useGrouping="mode == 'currency' ? '' : useGrouping"
          :currency="currency"
          :mode="mode"
          :suffix
          :prefix
        ...
        >
          ...
          <template #option="slotProps">
            <div class="flex space-x-1">
              <img
                :src="slotProps.option.flags.svg"
                width="30"
        ...
        <span v-else-if="type == 'groupcheckbox'">
          <div class="flex flex-wrap justify-content-center gap-3">
            <div
              class="flex h-8 space-x-1 items-center"
        ...
        <span v-if="type == 'file'">
          <span v-if="mode == 'advanced'">
            <FileUpload
              ...
              @select="input = multiple ? $event.files : $event.files[0]"
              class=""
              customUpload
            >
              <template #empty>
                ...
                >
                  <div class="flex gap-2">
                    <Button
                      @click="
                        {
                          !multiple ? (input = null) : undefined;
                          !multiple ? clearCallback() : undefined;
                          chooseCallback();
                        }
                      "
              ...
              <template #content="{ files, removeFileCallback }">
                <div v-if="files.length > 0">
                  <div class="grid w-full p-0 sm:p-1 gap-2">
                    ...
                    >
                      <div
                        class="flex space-x-2 cursor-default"
          ...
          <span v-else>
            <FileUpload
              ...
              :accept="acceptFile"
              :maxFileSize
              @input="input = $event.target.files[0]"
              class="w-full h-8"
              customUpload
            />
          </span>
