  ...
  for (let availability of props.availabilities) {
    horarios.value.push({
      ...
      days: [
        {
          day: "Lunes",
          times: availability.horarios
            .filter((day) => day.day == "Lunes")
            .map((time) => mapHorario(time)),
        },
        {
          day: "Martes",
          times: availability.horarios
            .filter((day) => day.day == "Martes")
            .map((time) => mapHorario(time)),
        },
        {
          day: "Miercoles",
          times: availability.horarios
            .filter((day) => day.day == "Miercoles")
            .map((time) => mapHorario(time)),
        },
        {
          day: "Jueves",
          times: availability.horarios
            .filter((day) => day.day == "Jueves")
            .map((time) => mapHorario(time)),
        },
        {
          day: "Viernes",
          times: availability.horarios
            .filter((day) => day.day == "Viernes")
            .map((time) => mapHorario(time)),
        },
        {
          day: "Sabado",
          times: availability.horarios
            .filter((day) => day.day == "Sabado")
            .map((time) => mapHorario(time)),
        },
        {
          day: "Domingo",
          times: availability.horarios
            .filter((day) => day.day == "Domingo")
            .map((time) => mapHorario(time)),
        },
        {
          day: "Festivo",
          times: availability.horarios
            .filter((day) => day.day == "Festivo")
            .map((time) => mapHorario(time)),
        },
      ],
    });
...
const submitPrice = () => {
  axios
    .post(route("availability.syncPrices", horarioSelect.value), {
  ...
  <div class="card">
          ...
          >
            <div class="flex justify-between w-full">
              <div class="flex flex-col w-full text-lg">
                <span class="font-bold whitespace-nowrap capitalize">{{
                  horario.name
                }}</span>
    ...
    <Modal v-model="showPrice" title="Lista de Precios" close-on-escape>
      <div class="card flex flex-col gap-y-2 justify-center w-full rounded-lg p-2">
        ...
        >
          <div
            v-if="formStart.price_type != 'duracion'"
          ...
          >
            ...
            <Input
              v-model="precio.min"
              type="number"
              :min="index == 0 ? 0 : precios[index - 1].max + 1"
